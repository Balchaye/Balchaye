...
<h3>Images From Our Website</h3>
     ...
     <marquee><?php  
        ...
        echo '<table>';
        $result=mysql_query("SELECT * FROM headerimage");

            while($row=mysql_fetch_array($result))
   {
      echo'<font charset=utf8>';
      echo "<td>";
          echo"<img style='margin-top:3px; border-width: 10px; margin-left:3px;' height=200 width=200  alt='Unable to View' src='" .$row['heade_image'] ."' >";
           echo'</font>';
     echo "</td>";
   }
        echo '</table>';
  ?></marquee>
    ...
    <marquee><?php  
        //$id = $_SESSION['log']['Username'];
        $query = mysql_query("SELECT * FROM projecttable ") or die (mysql_error()); 
        $row = mysql_fetch_array($query);
        echo '<table>';
